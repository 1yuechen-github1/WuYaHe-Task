...
def get_clo_list_houya(bone_len, min_val, max_val):
    # Interpolate on smoothed anchor colors to avoid visible banding.
    anchors = np.arange(6.0, 8.1, 0.1)
    ...
    kernel /= kernel.sum()
    rgb_smooth = np.empty_like(rgb_anchors)
    for c in range(3):
        ch = np.pad(rgb_anchors[:, c], (2, 2), mode="edge")
        rgb_smooth[:, c] = np.convolve(ch, kernel, mode="valid")

    x = float(np.clip(bone_len, anchors[0], anchors[-1]))
    r = np.interp(x, anchors, rgb_smooth[:, 0])
    g = np.interp(x, anchors, rgb_smooth[:, 1])
    b = np.interp(x, anchors, rgb_smooth[:, 2])
    return [r / 255.0, g / 255.0, b / 255.0]


def get_clo_list_qianya(bone_len, min_val, max_val):
    # Interpolate on smoothed anchor colors to avoid visible banding.
    anchors = np.arange(8.0, 12.1, 0.2)
    rgb_anchors = np.array([
        [253, 11, 0],
        [253, 39, 0],
        [253, 63, 1],
...
# ---- vis-debug specific functions kept from previous version ----
