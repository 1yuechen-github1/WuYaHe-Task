...
def extract_slice(ct_data, label_data, plane_coeffs, center_point, 
...
def merge_xiayuanxiang(nii_img, label_data, output_dir, label_pattern="*.nii.gz",label_affine=None,header=None,filename=None):
    ...
    参数:
    xiayuanxiang_files: 下颌数据文件路径列表
    label_dir: 标签数据目录路径
    output_dir: 输出目录路径
    label_pattern: 标签数据文件匹配模式
    """
    os.makedirs(output_dir, exist_ok=True)
    results = []
